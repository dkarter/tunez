defmodule TunezWeb.Artists.IndexLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app {assigns}>
      <.header responsive={false}>
        ...
        <:action>
          <.search_box data-role="artist-search" method="get" phx-submit="search" query={@query_text} />
        </:action>
        <:action>
  ...
  def pagination_links(assigns) do
    ~H"""
    <div
      :if={AshPhoenix.LiveView.prev_page?(@page) or AshPhoenix.LiveView.next_page?(@page)}
  ...
  defp query_string(page, query_text, sort_by, direction) do
    page
    |> AshPhoenix.LiveView.page_link_params(direction)
    |> case do
      :invalid -> []
      list -> list
    end
